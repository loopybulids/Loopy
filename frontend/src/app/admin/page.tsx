...
export default function CommandCenter() {
  ...
  /*
   * Paint the last payload instantly, then refresh — but only when it is for
   * the range being asked for. A remembered "Last 30 days" shown under a
   * "Yesterday" heading, with money on screen, is worse than a skeleton.
   */
  const [d, setD] = useState<any>(() => {
    if (typeof window === 'undefined') return null;
    try {
      const c = JSON.parse(localStorage.getItem(CACHE_KEY) || 'null');
      return c?.range && c.range.from === range.from && c.range.to === range.to ? c : null;
    } catch { return null; }
  });
  const [loadedFor, setLoadedFor] = useState(() => (d ? query : ''));
  const [err, setErr] = useState('');
  const [busy, setBusy] = useState(false);
  // Only the newest request may publish: pick two ranges quickly and the
  // slower, older answer must not land under the newer heading.
  ...
  const load = useCallback((force = false) => {
    ...
    api.adminCommand(q)
      .then((r) => {
        ...
        setD(r);
        setLoadedFor(q);
        try { localStorage.setItem(CACHE_KEY, JSON.stringify(r)); } catch { /* next visit just paints later */ }
      })
      .catch((e) => { if (latest.current === q) setErr(e?.message || 'Failed to load.'); })
      .finally(() => { if (latest.current === q) setBusy(false); });
  }, [query]);
  useEffect(() => { load(); }, [load]);

  if (err && !d) return <Card className="p-6 text-alert">{err} <button onClick={() => load(true)} className="ml-2 underline">Retry</button></Card>;
  ...
  const vsPrev = 'vs previous period';
  const label = rangeLabel(range);
  const monthly = d.range?.bucket === 'month';
  const stale = loadedFor !== query;
  /**
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-[13.5px] text-dim">{rangeDates(range)} · across every seller on Loopy</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            aria-label="Refresh"
            title="Refresh"
            className="grid h-9 w-9 place-items-center rounded-lg bg-accent text-white transition-colors hover:bg-accent-600 disabled:opacity-70"
          >
            <Icon name="refund" size={15} className={busy ? 'animate-spin' : ''} />
          </button>
        </div>
      </div>

      {err && (
        <p className="rounded-lg border border-alert/25 bg-alert-soft px-3.5 py-2.5 text-[12.5px] text-alert">
          Couldn’t load {label}: {err}
        </p>
      )}

      {/* Dimmed while a newly picked range loads, so figures for the old range
          are never read as figures for the new one. */}
      <div aria-busy={stale} className={`space-y-5 transition-opacity duration-200 ${stale ? 'pointer-events-none opacity-50' : ''}`}>
        ...
        */}
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          <Card className="p-5">
            ...
            <SectionTitle action={<span className="text-[11.5px] text-pale">{label}</span>}>Needs attention</SectionTitle>
            <ul className="space-y-2.5">
