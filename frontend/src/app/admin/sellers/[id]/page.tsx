...
export default function SellerProfile() {
  ...
  if (err) return <Card className="p-6 text-alert">{err} — <Link href="/admin/sellers" className="underline">back</Link></Card>;
  ...
  return (
    <div className="space-y-5">
      <Link href="/admin/sellers" className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-dim hover:text-slate"><Icon name="back" size={15} /> Sellers</Link>
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button onClick={() => impersonateSeller(id)} className="rounded-xl bg-accent px-4 py-2 text-[12.5px] font-bold text-white">Log in as seller →</button>
          {d.kycStatus !== 'approved' && <button onClick={() => kyc(true)} className="rounded-lg bg-accent px-4 py-2 text-[12.5px] font-semibold text-white transition-colors hover:bg-accent-600">Approve Seller</button>}
          {d.kycStatus !== 'rejected' && <button onClick={() => kyc(false)} className="rounded-xl bg-alert-soft px-4 py-2 text-[12.5px] font-bold text-alert">Suspend Seller</button>}
          <a href={d.email ? `mailto:${d.email}` : '#'} className="rounded-xl bg-cool px-4 py-2 text-[12.5px] font-bold text-slate">Message Seller</a>
        </div>
      </Card>
      ...
      <div className="inline-flex rounded-xl bg-white p-1 shadow-card">
        {(['overview', 'products', 'orders', 'payouts'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`rounded-lg px-4 py-2 text-[13px] font-bold capitalize ${tab === t ? 'bg-slate text-white' : 'text-dim'}`}>{t}</button>
        ))}
      </div>

      {tab === 'overview' && <Card className="p-5"><SectionTitle action={<span className="text-[12px] font-semibold text-accent">Last 14 days</span>}>Revenue Trend</SectionTitle><AreaTrend data={d.series} money height={240} /></Card>}
