  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 rounded-xl border border-hair bg-white px-3 py-2">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by store, username or email…" className="w-full bg-transparent text-[13.5px] outline-none placeholder:text-pale" />
      </div>
                ...
                <tr key={s.id} className="hover:bg-cool/60">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-slate text-[11px] font-extrabold text-white">{(s.storeName || '?').split(' ').map((w: string) => w[0]).join('').slice(0, 2).toUpperCase()}</span>
                      <div className="min-w-0"><Link href={`/admin/sellers/${s.id}`} className="block truncate font-bold text-slate hover:text-accent">{s.storeName}</Link><div className="truncate text-[11px] text-dim">{s.email || s.username}</div></div>
                    </div>
                  </td>
                  <td className="py-3 text-slate">{s.stats.products}</td>
                  <td className="py-3 text-slate">{s.stats.orders} <span className="text-pale">({s.stats.paidOrders} paid)</span></td>
                  <td className="py-3 text-slate">{s.stats.delivered}</td>
                  <td className="py-3 text-slate">{s.stats.customers}</td>
                  <td className="py-3 font-semibold text-slate">{money(s.stats.revenue)}</td>
                  <td className="py-3 text-accent">{money(s.stats.commission)}</td>
                  <td className="py-3">{statusChip(s.kycStatus)}</td>
                  <td className="py-3 pr-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      {s.kycStatus !== 'approved' && <button onClick={() => kyc(s.id, true)} className="rounded-lg bg-slate px-3 py-1.5 text-[11px] font-bold text-white">Approve</button>}
                      <button onClick={() => impersonateSeller(s.id)} className="rounded-lg bg-accent-soft px-3 py-1.5 text-[11px] font-bold text-accent">Log in</button>
                      <Link href={`/admin/sellers/${s.id}`} className="font-bold text-accent hover:underline">Open →</Link>
                    </div>
                  </td>
                </tr>
              ))}
              {!loading && !shown.length && <tr><td colSpan={9} className="px-4 py-10 text-center text-dim">No sellers found.</td></tr>}
              {loading && <tr><td colSpan={9} className="px-4 py-10 text-center text-dim animate-pulse">Loading…</td></tr>}
            </tbody>
          </table>
        </div>
        <div className="border-t border-hair px-4 py-3 text-[12px] text-dim">{shown.length} of {sellers.length} sellers</div>
      </Card>
    </div>
  );
}
